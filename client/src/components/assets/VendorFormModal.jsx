...
export function VendorFormModal({ isOpen, onClose, vendor = null, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        showSuccess(`Vendor '${result.name}' created successfully.`)
      }
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditing ? `Edit Vendor: ${vendor?.name}` : 'Register New Vendor'}
      maxWidth="max-w-lg"
    >
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                disabled={submitting}
                placeholder="Account Manager / Representative"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              />
              {fieldErrors.email && (
                <p className="text-[11px] text-red-600 mt-0.5">{fieldErrors.email}</p>
              )}
            </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Account Notes / SLA terms</label>
            <textarea
              rows={2}
