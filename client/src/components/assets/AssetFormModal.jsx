...
export function AssetFormModal({ isOpen, onClose, asset = null, onSaved }) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditing ? `Edit Asset: ${asset?.assetTag}` : 'Create New Asset'}
      maxWidth="max-w-2xl"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. AST-001 or LAPTOP-102"
              required={!isEditing}
              className={`w-full text-xs sm:text-sm border rounded-lg p-2 uppercase font-mono ${
                isEditing
                  ? 'bg-slate-100 border-slate-300 text-slate-500 cursor-not-allowed'
                  : 'bg-white border-slate-300 text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500'
              } ${fieldErrors.assetTag ? 'border-red-500' : ''}`}
            />
            {fieldErrors.assetTag && (
              <p className="text-[11px] text-red-600 mt-0.5">{fieldErrors.assetTag}</p>
            )}
            {isEditing && (
              <p className="text-[10px] text-slate-400 mt-0.5">
                Asset tag is an immutable identifier.
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              required
              className={`w-full text-xs sm:text-sm bg-white border rounded-lg p-2 text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500 ${
                fieldErrors.name ? 'border-red-500' : 'border-slate-300'
              }`}
            />
            {fieldErrors.name && (
              <p className="text-[11px] text-red-600 mt-0.5">{fieldErrors.name}</p>
            )}
          </div>
          ...
          <div>
            ...
            <select
              ...
              disabled={submitting}
              className="w-full text-xs sm:text-sm bg-white border border-slate-300 rounded-lg p-2 text-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500 cursor-pointer"
            >
          ...
          <div>
            ...
            <input
              ...
              disabled={submitting}
              placeholder="e.g. Ultrabook, Rackmount, Managed Switch"
              className="w-full text-xs sm:text-sm bg-white border border-slate-300 rounded-lg p-2 text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              disabled={submitting}
              placeholder="e.g. Lenovo, Dell, Apple, Cisco"
              className="w-full text-xs sm:text-sm bg-white border border-slate-300 rounded-lg p-2 text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
          ...
          {/* Model */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Model / Part Number</label>
            <input
              type="text"
              value={formData.model}
              onChange={(e) => handleChange('model', e.target.value)}
              disabled={submitting}
              placeholder="e.g. 21HM0004US, Latitude 5430"
              className="w-full text-xs sm:text-sm bg-white border border-slate-300 rounded-lg p-2 text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full text-xs sm:text-sm bg-white border border-slate-300 rounded-lg p-2 font-mono text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              disabled={submitting}
              placeholder="e.g. Building 2, Floor 3, IT Storeroom"
              className="w-full text-xs sm:text-sm bg-white border border-slate-300 rounded-lg p-2 text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              disabled={submitting}
              className="w-full text-xs sm:text-sm bg-white border border-slate-300 rounded-lg p-2 text-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
        ...
        {/* Description */}
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">Hardware Description</label>
          <textarea
            rows={2}
            value={formData.description}
            onChange={(e) => handleChange('description', e.target.value)}
            disabled={submitting}
            placeholder="Technical specifications, CPU, RAM, storage, peripherals..."
            className="w-full text-xs sm:text-sm bg-white border border-slate-300 rounded-lg p-2 text-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
        ...
        {/* Internal Notes */}
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">Inventory Notes</label>
          <textarea
            rows={2}
            value={formData.notes}
            onChange={(e) => handleChange('notes', e.target.value)}
            disabled={submitting}
            placeholder="Asset lifecycle notes, procurement order number, maintenance history..."
            className="w-full text-xs sm:text-sm bg-white border border-slate-300 rounded-lg p-2 text-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
